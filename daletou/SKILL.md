---
name: 大乐透
description: 大乐透走势数据分析工具。500.com（已废弃） 数据抓取，多维度分析（遗漏值、冷热号、奇偶比、大小比、跨度、和值、012路、前后区分离分析），最大覆盖策略推荐。Trigger: "分析大乐透" "推荐大乐透号码" "大乐透走势" "大乐透冷热号" "大乐透预测" "dlt分析" "超级大乐透"
...
从 500.com（已废弃） 抓取最近 100 期开奖数据，自动保存到 `~/.dlt_data/history.json`。
...
| 来源 | URL | 用途 |
|------|-----|------|
| 500.com（已废弃） 走势图 | `datachart.500.com（已废弃）/dlt/history/newinc/history.php` | 主数据源 |
